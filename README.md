...
<details><a name="слой-модели"></a>
  ...
  <ul>
    ...
    <li>Базовый класс бизнес-модели <code>Model&ltT&gt</code>
      <p>От этого класса наследуется <code>AppState</code>. Конструктор принимает начальный данные для модели и объект событий для уведомления об изменениях в модели. Класс содержит метод <code>emitChange</code>, для того чтобы вызывать событие из компонента.</p>
...
<details><a name="слой-представления"></a>
  ...
  <ul>
    ...
    <li>Класс <code>Card</code> расширяет базовый класс <code>Component&ltT&gt</code>
